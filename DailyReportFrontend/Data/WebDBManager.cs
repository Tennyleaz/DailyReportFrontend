...
namespace DailyReportFrontend.Data
{
    class WebDBManager : IDisposable
    {
        ...
        public async Task<List<Project>> ReadAllProjectsAsync()
        {
            if (_httpClient == null)
                return null;

            return await _httpClient.QueryAllAsync<Project>("Project");
        }
        ...
        public async Task<Project> ReadProjectAsync(int id)
        {
            ...
            return await _httpClient.QueryByIdAsync<Project>("Project", id);
        }

        public async Task<DailyReport> ReadDailyReportAsync(int id)
        {
            if (_httpClient == null)
                return null;

            return await _httpClient.QueryByIdAsync<DailyReport>("DailyReport", id);
        }
        ...
        //public async Task<PeriodReport> ReadPeriodReportAsync(int id)
        //{
        //    if (_httpClient == null)
        //        return null;
        //    return await _httpClient.QueryByIdAsync<PeriodReport>("PeriodReport", id);
        //}
        ...
        public async Task<int> TryAddProjectAsync(Project report)
        {
            if (report == null) return -1;

            try
            {
                report = await _httpClient.HttpPostAsync<Project>(report, "Project/TryAdd");
                return report.Id;
            }
            catch (Exception ex)
            {
